...
<section class="relative py-28 bg-gray-900">
  <div class="relative z-10 max-w-screen-xl mx-auto text-gray-600 sm:px-4 md:px-8">
    <div class="max-w-lg space-y-3 px-4 sm:mx-auto sm:text-center sm:px-0">
        <a
            href="{{URL::tokenRoute('collection.index')}}"
      ...
      <p class="text-gray-300">
        Please fill out the form bellow to create collection.
      </p>
    </div>
    <div class="mt-12 mx-auto px-4 p-8 bg-white sm:max-w-lg sm:px-8 sm:rounded-xl">
      <form method="POST" action="{{ route('collection.store') }}" class="space-y-5">
        <div>
          <label for="collection_name" class="font-medium">
            name
          </label>
          <input
            type="text"
        ...
        <div>
          <label for="collection_description" class="font-medium">
            Description
          </label>
          <textarea required id="collection_description" name="description" class="w-full mt-2 h-36 px-3 py-2 resize-none appearance-none bg-transparent outline-none border focus:border-gray-800 shadow-sm rounded-lg"></textarea>
        </div>
        <button
